...
$pdo = db_conn();
//２．データ登録SQL作成
$stmt = $pdo->prepare("SELECT * FROM crud WHERE lid=:lid");
...
<header>
  <nav class="navbar navbar-default">
    <div class="container-fluid">
      ...
      <a class="navbar-brand" href="select.php">Archive</a>
...
<script>
  ...
  const rolechecker = [ 'learner', 'student', 'tutor', 'recruiter']
  let icon;

  // それぞれの職業に合わせたアイコンを表示
  if (rolechecker[prof[1]] === 'learner' ){
    icon = '<i class="fas fa-user-shield"></i>'
  }else if (rolechecker[prof[1]] === 'student'){
    icon = '<i class="fas fa-graduation-cap"></i>'
  }else if (rolechecker[prof[1]] === 'tutor'){
    icon = '<i class="fas fa-user-graduate"></i>'
  }else if (rolechecker[prof[1]] = 'recruiter'){
